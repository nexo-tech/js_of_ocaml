...
#### Task 14.3: Runtime Primitive Linking 🔴 **HIGH**

**Problem**: Generated code calls `caml_*` primitives but runtime modules provide `M.function` exports. Need automatic linking without code duplication or heavy refactoring.

**Goal**: Implement compiler/linking infrastructure to automatically bridge `M.function` → `caml_*` calls, verify PRIMITIVES.md coverage, implement missing primitives in runtime modules using `M.*` pattern, and ensure hello_lua example works.

**Requirements**:
- Zero code duplication
- No heavy refactoring of existing runtime modules
- Reuse DEPS.md linking infrastructure
- All existing tests pass
- hello_lua example runs successfully

See LINKING.md for detailed implementation plan.
