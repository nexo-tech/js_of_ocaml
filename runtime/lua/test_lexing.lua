...
-- Load lexing.lua directly (it defines global caml_* functions)
dofile("lexing.lua")
...
test("lex_array: empty array", function()
  local arr = caml_lex_array({})
  assert_eq(#arr, 0)
end)

test("lex_array: single element", function()
  local arr = caml_lex_array({0x42, 0x00})
  assert_eq(#arr, 1)
  assert_eq(arr[1], 0x42)
end)

test("lex_array: multiple elements", function()
  local arr = caml_lex_array({0x01, 0x00, 0x02, 0x00, 0x03, 0x00})
  assert_eq(#arr, 3)
  assert_eq(arr[1], 1)
  assert_eq(arr[2], 2)
  assert_eq(arr[3], 3)
...
test("lex_array: negative values (sign extension)", function()
  ...
  local arr = caml_lex_array({0xFF, 0xFF})
  assert_eq(#arr, 1)
  assert_eq(arr[1], -1)
end)

test("lex_array: mixed positive and negative", function()
  local arr = caml_lex_array({0x01, 0x00, 0xFF, 0xFF, 0x00, 0x80})
  assert_eq(#arr, 3)
  assert_eq(arr[1], 1)
  assert_eq(arr[2], -1)
...
test("create from string: initial positions", function()
  local lexbuf = caml_create_lexbuf_from_string("test")
...
test("create from byte array", function()
  ...
  local lexbuf = caml_create_lexbuf_from_string(bytes)
  assert_eq(lexbuf[3], 5)  -- buffer_len
end)
...
test("lexeme: extract substring", function()
  local lexbuf = caml_create_lexbuf_from_string("hello world")
  lexbuf[5] = 0  -- start_pos
  lexbuf[6] = 5  -- curr_pos (points after "hello")

  local lexeme = caml_lexeme(lexbuf)
...
test("lexeme_string: extract as Lua string", function()
  ...
  local str = caml_lexeme_string(lexbuf)
  assert_eq(str, "hello")
end)
...
test("lexeme_start: absolute position", function()
  ...
  local start = caml_lexeme_start(lexbuf)
...
test("lexeme_end_p: returns current position record", function()
  local lexbuf = caml_create_lexbuf_from_string("test")
  local curr_p = caml_lexeme_end_p(lexbuf)
...
test("new_line: increments line number", function()
  local lexbuf = caml_create_lexbuf_from_string("line1\nline2")
  local curr_p = lexbuf[12]

  assert_eq(curr_p.pos_lnum, 1)
  caml_new_line(lexbuf)
...
test("lexeme_char: get character at offset", function()
  ...
  assert_eq(caml_lexeme_char(lexbuf, 0), 104)  -- 'h'
  assert_eq(caml_lexeme_char(lexbuf, 1), 101)  -- 'e'
  assert_eq(caml_lexeme_char(lexbuf, 4), 111)  -- 'o'
end)

test("lexeme_char: out of bounds raises error", function()
  local lexbuf = caml_create_lexbuf_from_string("hi")
  lexbuf[5] = 0
  lexbuf[6] = 2

  local success, err = pcall(function()
    caml_lexeme_char(lexbuf, 5)
  end)
...
test("lex_engine: table caching", function()
  ...
  local lexbuf = caml_create_lexbuf_from_string("a")
  pcall(function() caml_lex_engine(tbl, 0, lexbuf) end)
...
test("lex_engine: reentry state handling", function()
  ...
  local lexbuf = caml_create_lexbuf_from_string("x")

  -- Call with negative state (simulating reentry)
  pcall(function() caml_lex_engine(tbl, -2, lexbuf) end)
...
test("integration: manual lexeme extraction", function()
  ...
  local input = "ab"
  local lexbuf = caml_create_lexbuf_from_string(input)

  -- Manually advance position as if lexer matched 1 character
  lexbuf[6] = 1  -- curr_pos

  local lexeme = caml_lexeme_string(lexbuf)
  assert_eq(lexeme, "a")
end)
...
test("integration: multiple lexeme extractions", function()
  ...
  lexbuf[5] = 0
  lexbuf[6] = 3
  local lex1 = caml_lexeme_string(lexbuf)
  assert_eq(lex1, "abc")

  -- Second lexeme: "def"
  lexbuf[5] = 4
  lexbuf[6] = 7
  local lex2 = caml_lexeme_string(lexbuf)
  assert_eq(lex2, "def")

  -- Third lexeme: "ghi"
  lexbuf[5] = 8
  lexbuf[6] = 11
  local lex3 = caml_lexeme_string(lexbuf)
  assert_eq(lex3, "ghi")
end)
