#!/usr/bin/env lua
-- Test suite for array.lua array operations module
...
test("Make empty array", function()
  local arr = array.make(0, nil)
  assert(array.length(arr) == 0, "Empty array should have length 0")
end)

-- Test length
test("Array length", function()
  local arr = array.make(10, 0)
  assert(array.length(arr) == 10, "Length should be 10")
end)

-- Test get/set with bounds checking
test("Get element", function()
  local arr = array.make(3, 0)
  arr[1] = 10
  arr[2] = 20
  arr[3] = 30
  assert(array.get(arr, 0) == 10, "Get index 0")
  assert(array.get(arr, 1) == 20, "Get index 1")
  assert(array.get(arr, 2) == 30, "Get index 2")
end)

test("Get out of bounds", function()
  local arr = array.make(3, 0)
  local success = pcall(function() array.get(arr, 5) end)
  assert(not success, "Should fail for index >= length")
  success = pcall(function() array.get(arr, -1) end)
...
test("Set out of bounds", function()
  local arr = array.make(3, 0)
  local success = pcall(function() array.set(arr, 5, 999) end)
  assert(not success, "Should fail for index >= length")
  success = pcall(function() array.set(arr, -1, 999) end)
  assert(not success, "Should fail for negative index")
end)
...
test("Unsafe get", function()
  local arr = array.make(3, 0)
  arr[2] = 42
  assert(array.unsafe_get(arr, 1) == 42, "Unsafe get should work")
end)

test("Unsafe set", function()
  local arr = array.make(3, 0)
  array.unsafe_set(arr, 1, 99)
...
test("Array of list", function()
  ...
  local arr = array.of_list(list)
  assert(array.length(arr) == 3, "Array should have length 3")
...
test("Array of empty list", function()
  local arr = array.of_list(0)  -- Empty list
  assert(array.length(arr) == 0, "Should be empty")
end)
...
test("Sub array", function()
  local arr = array.make(5, 0)
  for i = 0, 4 do
    arr[i + 1] = i * 10
  end
  local sub = array.sub(arr, 1, 3)  -- Extract [10, 20, 30]
  assert(array.length(sub) == 3, "Sub should have length 3")
...
test("Append arrays", function()
  local arr1 = array.make(2, 0)
  arr1[1] = 1
  arr1[2] = 2
  local arr2 = array.make(3, 0)
  arr2[1] = 3
  arr2[2] = 4
  arr2[3] = 5
  local result = array.append(arr1, arr2)
  assert(array.length(result) == 5, "Result should have length 5")
...
test("Append with empty", function()
  local arr = array.make(2, 1)
  local empty = array.make(0, nil)
  local result = array.append(arr, empty)
  assert(array.length(result) == 2, "Should keep original length")
  result = array.append(empty, arr)
  assert(array.length(result) == 2, "Should keep original length")
end)

-- Test concat
test("Concat arrays", function()
  local a1 = array.make(2, 0)
  a1[1] = 1
  a1[2] = 2
  local a2 = array.make(1, 0)
  a2[1] = 3
  local a3 = array.make(2, 0)
  a3[1] = 4
  a3[2] = 5
  ...
  local result = array.concat(list)
  assert(array.length(result) == 5, "Concat should have length 5")
  assert(result[1] == 1, "Element 0")
  assert(result[5] == 5, "Element 4")
end)

test("Concat empty list", function()
  local result = array.concat(0)  -- Empty list
  assert(array.length(result) == 0, "Should be empty")
end)

-- Test blit
test("Blit arrays", function()
  local src = array.make(5, 0)
  for i = 0, 4 do
    src[i + 1] = i + 1
  end
  local dst = array.make(5, 99)
  array.blit(src, 1, dst, 0, 3)  -- Copy [2,3,4] to start of dst
...
test("Blit overlapping forward", function()
  ...
  array.blit(arr, 0, arr, 2, 3)  -- Copy [0,1,2] to positions 2,3,4
...
test("Blit overlapping backward", function()
  ...
  array.blit(arr, 2, arr, 0, 3)  -- Copy [2,3,4] to positions 0,1,2
...
test("Fill array", function()
  local arr = array.make(5, 0)
  array.fill(arr, 1, 3, 42)  -- Fill positions 1,2,3 with 42
...
test("Init array with function", function()
  local arr = array.init(5, function(i) return i * i end)
  assert(array.length(arr) == 5, "Length should be 5")
  assert(arr[1] == 0, "0^2")
  assert(arr[2] == 1, "1^2")
  assert(arr[3] == 4, "2^2")
  assert(arr[4] == 9, "3^2")
...
test("Iter over array", function()
  ...
  arr[3] = 3
  local sum = 0
  array.iter(function(x) sum = sum + x end, arr)
  assert(sum == 6, "Sum should be 6")
end)

test("Iteri with index", function()
  local arr = array.make(3, 10)
  local indices = {}
  array.iteri(function(i, x) table.insert(indices, i) end, arr)
...
test("Map over array", function()
  ...
  arr[2] = 2
  arr[3] = 3
  local result = array.map(function(x) return x * 2 end, arr)
  assert(array.length(result) == 3, "Same length")
...
test("Mapi with index", function()
  local arr = array.make(3, 10)
  local result = array.mapi(function(i, x) return i + x end, arr)
...
test("Fold left", function()
  local arr = array.make(4, 0)
  arr[1] = 1
  arr[2] = 2
  arr[3] = 3
  arr[4] = 4
  local result = array.fold_left(function(acc, x) return acc + x end, 0, arr)
...
test("Primitives are registered", function()
  ...
  assert(core.get_primitive("caml_make_vect") == array.make, "make_vect registered")
  assert(core.get_primitive("caml_array_get") == array.get, "get registered")
  assert(core.get_primitive("caml_array_set") == array.set, "set registered")
  assert(core.get_primitive("caml_array_blit") == array.blit, "blit registered")
end)

-- Test module registration
test("Module is registered", function()
  local core = require("core")
  local mod = core.get_module("array")
  assert(mod == array, "array module should be registered")
end)
