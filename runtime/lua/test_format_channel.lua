...
-- Load io.lua (defines global caml_* functions)
dofile("io.lua")

-- Load format.lua (defines global caml_* functions)
dofile("format.lua")
...
test("fprintf: write integer to file", function()
  ...
  local fd = caml_sys_open(temp_file, make_ocaml_list({1, 6}), 438)  -- O_WRONLY|O_CREAT, 0666
...
test("fprintf: write multiple values", function()
  ...
  local fd = caml_sys_open(temp_file, make_ocaml_list({1, 6}), 438)
  local chanid = caml_ml_open_descriptor_out(fd)
...
test("fscanf: read integer from file", function()
  ...
  -- Open for reading
  local fd = caml_sys_open(temp_file, make_ocaml_list({0}), 438)  -- O_RDONLY
...
test("fscanf: read multiple integers", function()
  ...
  local fd = caml_sys_open(temp_file, make_ocaml_list({0}), 438)
  local chanid = caml_ml_open_descriptor_in(fd)
...
test("Round-trip: integer", function()
  ...
  -- Write
  local fd_w = caml_sys_open(temp_file, make_ocaml_list({1, 6}), 438)
  local chan_w = caml_ml_open_descriptor_out(fd_w)
  caml_fprintf(chan_w, "%d\n", 12345)
  caml_ml_close_channel(chan_w)
  caml_sys_close(fd_w)

  -- Read
  local fd_r = caml_sys_open(temp_file, make_ocaml_list({0}), 438)
  local chan_r = caml_ml_open_descriptor_in(fd_r)
  local result = caml_fscanf(chan_r, "%d")
  assert_eq(result[1], 12345)
  caml_ml_close_channel(chan_r)
  caml_sys_close(fd_r)
