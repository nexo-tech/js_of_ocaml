...
test("caml_sys_get_config returns config tuple", function()
  local config = sys.caml_sys_get_config(core.unit)
...
test("caml_sys_argv returns array", function()
  local argv = sys.caml_sys_argv(core.unit)
...
test("caml_sys_get_argv returns tuple", function()
  local result = sys.caml_sys_get_argv(core.unit)
...
test("caml_sys_executable_name returns program name", function()
  local name = sys.caml_sys_executable_name(core.unit)
...
test("caml_sys_modify_argv changes argv", function()
  local old_argv = sys.caml_sys_argv(core.unit)
  local new_argv = {tag = 0, [1] = "test_prog"}
  sys.caml_sys_modify_argv(new_argv)
  local current_argv = sys.caml_sys_argv(core.unit)
  assert_equal(current_argv[1], new_argv[1], "argv should be modified")
  -- Restore
  sys.caml_sys_modify_argv(old_argv)
end)
...
test("caml_set_static_env and caml_sys_getenv", function()
  ...
  sys.caml_set_static_env(key, value)
  local result = sys.caml_sys_getenv(key)
...
test("caml_sys_getenv with system env var", function()
  ...
  local path_key = "PATH"
  local ok, result = pcall(sys.caml_sys_getenv, path_key)
...
test("caml_sys_getenv_opt returns Some for existing var", function()
  ...
  sys.caml_set_static_env(key, value)
  local result = sys.caml_sys_getenv_opt(key)
...
test("caml_sys_getenv_opt returns None for nonexistent var", function()
  ...
  local result = sys.caml_sys_getenv_opt(key)
  assert_equal(result, core.unit, "should return None (0)")
end)

-- Test 12: caml_sys_time
test("caml_sys_time returns non-negative number", function()
  local t1 = sys.caml_sys_time(core.unit)
  ...
  local t2 = sys.caml_sys_time(core.unit)
...
test("caml_sys_time_include_children returns time", function()
  local t = sys.caml_sys_time_include_children(core.unit)
...
test("caml_sys_random_seed returns array of 4 integers", function()
  local seed = sys.caml_sys_random_seed(core.unit)
...
test("caml_sys_const_big_endian", function()
  assert_equal(sys.caml_sys_const_big_endian(core.unit), 0, "should be little endian")
end)

test("caml_sys_const_word_size", function()
  assert_equal(sys.caml_sys_const_word_size(core.unit), 32, "word size should be 32")
end)

test("caml_sys_const_int_size", function()
  assert_equal(sys.caml_sys_const_int_size(core.unit), 32, "int size should be 32")
end)

test("caml_sys_const_max_wosize", function()
  local max_wosize = sys.caml_sys_const_max_wosize(core.unit)
  assert_equal(max_wosize, math.floor(0x7fffffff / 4), "max_wosize should be correct")
end)

test("caml_sys_const_backend_type", function()
  local backend = sys.caml_sys_const_backend_type(core.unit)
...
test("caml_sys_file_exists with existing file", function()
  ...
  local result = sys.caml_sys_file_exists(name)
  assert_equal(result, core.true_val, "file should exist")
...
test("caml_sys_file_exists with nonexistent file", function()
  ...
  local result = sys.caml_sys_file_exists(name)
  assert_equal(result, core.false_val, "file should not exist")
end)
...
test("caml_sys_rename renames file", function()
  ...
  sys.caml_sys_rename(old_name, new_name)
...
test("caml_sys_is_regular_file detects regular file", function()
  ...
  local name = test_file
  local result = sys.caml_sys_is_regular_file(name)
  assert_equal(result, core.true_val, "should detect regular file")
...
test("caml_sys_is_directory detects directory", function()
  ...
  local name = "/tmp"
  local result = sys.caml_sys_is_directory(name)
  -- This may fail without lfs or on Windows, so we just check it doesn't crash
  assert_true(result == core.true_val or result == core.false_val, "should return boolean")
end)
...
test("caml_sys_system_command executes command", function()
  ...
  local result = sys.caml_sys_system_command(cmd)
...
test("caml_sys_isatty returns false", function()
  local result = sys.caml_sys_isatty(0)
  assert_equal(result, core.false_val, "should return false")
end)

-- Test 29: caml_runtime_variant
test("caml_runtime_variant returns empty string", function()
  local result = sys.caml_runtime_variant(core.unit)
...
test("caml_runtime_parameters returns empty string", function()
  local result = sys.caml_runtime_parameters(core.unit)
  assert_ocaml_string(result, "", "should return empty string")
end)

-- Test 31: Runtime warnings
test("runtime warnings can be enabled and disabled", function()
  sys.caml_ml_enable_runtime_warnings(core.true_val)
  assert_equal(sys.caml_ml_runtime_warnings_enabled(core.unit), core.true_val, "should be enabled")
  sys.caml_ml_enable_runtime_warnings(core.false_val)
  assert_equal(sys.caml_ml_runtime_warnings_enabled(core.unit), core.false_val, "should be disabled")
end)

-- Test 32: caml_sys_io_buffer_size
test("caml_sys_io_buffer_size returns 65536", function()
  local size = sys.caml_sys_io_buffer_size(core.unit)
...
test("caml_sys_temp_dir_name returns temp directory", function()
  local temp = sys.caml_sys_temp_dir_name(core.unit)
  local temp_str = temp
...
test("caml_xdg_defaults returns empty list", function()
  local result = sys.caml_xdg_defaults(core.unit)
  assert_equal(result, core.unit, "should return empty list (0)")
end)

-- Test 35: Signal number conversion
test("caml_sys_convert_signal_number returns same number", function()
  assert_equal(sys.caml_sys_convert_signal_number(15), 15, "should return same")
  assert_equal(sys.caml_sys_rev_convert_signal_number(15), 15, "should return same")
end)
...
test("OS type constants are consistent", function()
  ...
  local is_unix = sys.caml_sys_const_ostype_unix(core.unit)
  local is_win32 = sys.caml_sys_const_ostype_win32(core.unit)
  local is_cygwin = sys.caml_sys_const_ostype_cygwin(core.unit)

  if os_type == "Unix" then
    assert_equal(is_unix, core.true_val, "Unix flag should be true")
    assert_equal(is_win32, core.false_val, "Win32 flag should be false")
  elseif os_type == "Win32" then
    assert_equal(is_unix, core.false_val, "Unix flag should be false")
    assert_equal(is_win32, core.true_val, "Win32 flag should be true")
  end
  assert_equal(is_cygwin, core.false_val, "Cygwin flag should always be false")
end)

-- Test 37: caml_install_signal_handler (no-op)
test("caml_install_signal_handler is no-op", function()
  local result = sys.caml_install_signal_handler(15, 0)
  assert_equal(result, core.unit, "should return unit")
end)
...
test("file operations with special characters", function()
  ...
  if f then
    ...
    local name = test_file
    local exists = sys.caml_sys_file_exists(name)
    assert_equal(exists, core.true_val, "should handle paths with spaces")
