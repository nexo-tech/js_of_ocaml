...
test("create: empty hash table", function()
  local h = hashtbl.caml_hash_create()
  assert_eq(hashtbl.caml_hash_length(h), 0)
end)

test("create: with initial size", function()
  local h = hashtbl.caml_hash_create(32)
  assert_eq(hashtbl.caml_hash_length(h), 0)
  local stats = hashtbl.caml_hash_stats(h)
  assert_eq(stats.capacity, 32)
end)

test("create: with invalid size defaults to 16", function()
  local h = hashtbl.caml_hash_create(0)
  local stats = hashtbl.caml_hash_stats(h)
...
test("add: single binding", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "value1")
  assert_eq(hashtbl.caml_hash_length(h), 1)
end)

test("add: multiple bindings", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "value1")
  hashtbl.caml_hash_add(h, "key2", "value2")
  hashtbl.caml_hash_add(h, "key3", "value3")
  assert_eq(hashtbl.caml_hash_length(h), 3)
end)

test("add: duplicate keys (both bindings kept)", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "value1")
  hashtbl.caml_hash_add(h, "key1", "value2")
  assert_eq(hashtbl.caml_hash_length(h), 2)
end)

test("add: integer keys", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, 1, "one")
  hashtbl.caml_hash_add(h, 2, "two")
  hashtbl.caml_hash_add(h, 3, "three")
  assert_eq(hashtbl.caml_hash_length(h), 3)
end)

test("add: mixed types", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, 1, "number")
  hashtbl.caml_hash_add(h, "str", "string")
  hashtbl.caml_hash_add(h, 3.14, "float")
  assert_eq(hashtbl.caml_hash_length(h), 3)
end)
...
test("find: existing key", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "value1")
  local value = hashtbl.caml_hash_find(h, "key1")
...
test("find: non-existent key raises error", function()
  ...
  assert_error(function()
    hashtbl.caml_hash_find(h, "missing")
  end, "Not_found")
end)

test("find: most recent binding with duplicate keys", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "old")
  hashtbl.caml_hash_add(h, "key1", "new")
  local value = hashtbl.caml_hash_find(h, "key1")
  assert_eq(value, "new")
end)
...
test("find: integer keys", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, 42, "answer")
  local value = hashtbl.caml_hash_find(h, 42)
...
test("find_opt: existing key", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "value1")
  local value = hashtbl.caml_hash_find_opt(h, "key1")
  assert_eq(value, "value1")
end)

test("find_opt: non-existent key returns nil", function()
  local h = hashtbl.caml_hash_create()
  local value = hashtbl.caml_hash_find_opt(h, "missing")
  assert_eq(value, nil)
end)
...
test("remove: existing key", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "value1")
  hashtbl.caml_hash_remove(h, "key1")
  assert_eq(hashtbl.caml_hash_length(h), 0)
end)
...
test("remove: non-existent key (no effect)", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "value1")
  hashtbl.caml_hash_remove(h, "missing")
  assert_eq(hashtbl.caml_hash_length(h), 1)
end)
...
test("replace: new key", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_replace(h, "key1", "value1")
  assert_eq(hashtbl.caml_hash_length(h), 1)
  local value = hashtbl.caml_hash_find(h, "key1")
  assert_eq(value, "value1")
end)
...
test("replace: existing key", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "old")
  hashtbl.caml_hash_replace(h, "key1", "new")
  assert_eq(hashtbl.caml_hash_length(h), 1)
  local value = hashtbl.caml_hash_find(h, "key1")
  assert_eq(value, "new")
end)
...
test("replace: removes all duplicate bindings", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "old1")
  hashtbl.caml_hash_add(h, "key1", "old2")
  hashtbl.caml_hash_add(h, "key1", "old3")
  hashtbl.caml_hash_replace(h, "key1", "new")
  assert_eq(hashtbl.caml_hash_length(h), 1)
  local value = hashtbl.caml_hash_find(h, "key1")
  assert_eq(value, "new")
...
test("mem: existing key", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "value1")
  assert_true(hashtbl.caml_hash_mem(h, "key1"))
end)

test("mem: non-existent key", function()
  local h = hashtbl.caml_hash_create()
  assert_false(hashtbl.caml_hash_mem(h, "missing"))
end)
...
test("mem: after remove", function()
  ...
  hashtbl.caml_hash_add(h, "key1", "value1")
  hashtbl.caml_hash_remove(h, "key1")
  assert_false(hashtbl.caml_hash_mem(h, "key1"))
end)
...
test("length: tracks additions", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "k1", "v1")
  assert_eq(hashtbl.caml_hash_length(h), 1)
  hashtbl.caml_hash_add(h, "k2", "v2")
  assert_eq(hashtbl.caml_hash_length(h), 2)
  hashtbl.caml_hash_add(h, "k3", "v3")
  assert_eq(hashtbl.caml_hash_length(h), 3)
end)
...
test("length: tracks removals", function()
  ...
  hashtbl.caml_hash_add(h, "k1", "v1")
  hashtbl.caml_hash_add(h, "k2", "v2")
  hashtbl.caml_hash_remove(h, "k1")
  assert_eq(hashtbl.caml_hash_length(h), 1)
end)

test("length: includes duplicate keys", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "key1", "v1")
  hashtbl.caml_hash_add(h, "key1", "v2")
  assert_eq(hashtbl.caml_hash_length(h), 2)
end)
...
test("clear: can reuse hash table", function()
  ...
  hashtbl.caml_hash_add(h, "k2", "v2")
  assert_eq(hashtbl.caml_hash_length(h), 1)
  local value = hashtbl.caml_hash_find(h, "k2")
  assert_eq(value, "v2")
end)
...
test("iter: empty hash table", function()
  local h = hashtbl.caml_hash_create()
  local count = 0
  hashtbl.caml_hash_iter(h, function(k, v)
    count = count + 1
  end)
  assert_eq(count, 0)
end)
...
test("fold: empty hash table", function()
  local h = hashtbl.caml_hash_create()
  local result = hashtbl.caml_hash_fold(h, function(k, v, acc)
    return acc + 1
  end, 0)
  assert_eq(result, 0)
end)
...
test("fold: count elements", function()
  ...
  hashtbl.caml_hash_add(h, "k2", "v2")
  hashtbl.caml_hash_add(h, "k3", "v3")
  local count = hashtbl.caml_hash_fold(h, function(k, v, acc)
    return acc + 1
  end, 0)
  assert_eq(count, 3)
end)
...
test("fold: sum integer values", function()
  local h = hashtbl.caml_hash_create()
  hashtbl.caml_hash_add(h, "k1", 10)
  hashtbl.caml_hash_add(h, "k2", 20)
  hashtbl.caml_hash_add(h, "k3", 30)
  local sum = hashtbl.caml_hash_fold(h, function(k, v, acc)
    return acc + v
  end, 0)
  assert_eq(sum, 60)
end)
...
test("entries: empty hash table", function()
  local h = hashtbl.caml_hash_create()
  local count = 0
  for k, v in hashtbl.caml_hash_entries(h) do
    count = count + 1
  end
  assert_eq(count, 0)
end)
...
test("keys: empty hash table", function()
  local h = hashtbl.caml_hash_create()
  local keys = hashtbl.caml_hash_keys(h)
  assert_eq(#keys, 0)
end)
...
test("values: empty hash table", function()
  local h = hashtbl.caml_hash_create()
  local values = hashtbl.caml_hash_values(h)
  assert_eq(#values, 0)
end)
...
test("to_array: empty hash table", function()
  local h = hashtbl.caml_hash_create()
  local arr = hashtbl.caml_hash_to_array(h)
  assert_eq(#arr, 0)
end)
...
test("resize: automatic on load factor", function()
  local h = hashtbl.caml_hash_create(4)
  -- Add enough elements to trigger resize (load factor > 0.75)
  for i = 1, 10 do
    hashtbl.caml_hash_add(h, "key" .. i, "value" .. i)
  end
  assert_eq(hashtbl.caml_hash_length(h), 10)
  -- All elements should still be findable
  for i = 1, 10 do
    local value = hashtbl.caml_hash_find(h, "key" .. i)
    assert_eq(value, "value" .. i)
  end
end)

test("resize: preserves all bindings", function()
  local h = hashtbl.caml_hash_create(2)
  for i = 1, 20 do
    hashtbl.caml_hash_add(h, i, i * 10)
  end
  assert_eq(hashtbl.caml_hash_length(h), 20)
  for i = 1, 20 do
    local value = hashtbl.caml_hash_find(h, i)
...
test("complex: OCaml byte array keys", function()
  ...
  hashtbl.caml_hash_add(h, key1, "greeting")
  hashtbl.caml_hash_add(h, key2, "earth")
  local v1 = hashtbl.caml_hash_find(h, key1)
  local v2 = hashtbl.caml_hash_find(h, key2)
...
test("complex: table keys with same structure", function()
  ...
  hashtbl.caml_hash_add(h, key1, "first")
  local value = hashtbl.caml_hash_find(h, key2)
...
test("performance: many insertions", function()
  local h = hashtbl.caml_hash_create()
  for i = 1, 1000 do
    hashtbl.caml_hash_add(h, i, i * 2)
  end
  assert_eq(hashtbl.caml_hash_length(h), 1000)
end)
...
test("performance: mixed operations", function()
  local h = hashtbl.caml_hash_create()
  for i = 1, 100 do
    hashtbl.caml_hash_add(h, i, i)
  end
  for i = 1, 50 do
    hashtbl.caml_hash_remove(h, i)
  end
  for i = 51, 100 do
    assert_true(hashtbl.caml_hash_mem(h, i))
  end
  assert_eq(hashtbl.caml_hash_length(h), 50)
end)
