...
function M.caml_lex_engine(tbl, start_state, lexbuf)
  -- Parse and cache transition tables on first use
  if not tbl.lex_default then
    tbl.lex_base = M.caml_lex_array(tbl[TBL_BASE])
    tbl.lex_backtrk = M.caml_lex_array(tbl[TBL_BACKTRK])
    tbl.lex_check = M.caml_lex_array(tbl[TBL_CHECK])
    tbl.lex_trans = M.caml_lex_array(tbl[TBL_TRANS])
    tbl.lex_default = M.caml_lex_array(tbl[TBL_DEFAULT])
  end
...
function M.caml_create_lexbuf_from_string(s)
...
-- Returns: lexbuf structure
function M.caml_create_lexbuf_from_channel(channel_id)
  ...
  local function refill_func(lexbuf)
    local buf_size = 1024
    local buffer = {}
    local n = io_module.caml_ml_input(channel_id, buffer, 0, buf_size)
...
-- lexbuf: lexer buffer
-- Returns: Lua string
function M.caml_lexeme_string(lexbuf)
  local bytes = M.caml_lexeme(lexbuf)
  local chars = {}
  for i = 1, #bytes do
...
-- lexbuf: lexer buffer
-- Returns: integer
function M.caml_lexeme_start(lexbuf)
...
-- lexbuf: lexer buffer
-- Returns: integer
function M.caml_lexeme_end(lexbuf)
...
function M.caml_lexeme_start_p(lexbuf)
...
function M.caml_lexeme_char(lexbuf, n)
...
-- Flush lexbuf buffer
-- lexbuf: lexer buffer
function M.caml_flush_lexbuf(lexbuf)
