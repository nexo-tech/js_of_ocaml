...
function M.caml_parse_engine(tables, env, cmd, arg)
  -- Lazy load lexing module for caml_lex_array
  local lexing_module = package.loaded.lexing or require("lexing")

  -- Parse and cache tables on first use
  if not tables.dgoto then
    tables.defred = lexing_module.caml_lex_array(tables[TBL_DEFRED])
    tables.sindex = lexing_module.caml_lex_array(tables[TBL_SINDEX])
    tables.check = lexing_module.caml_lex_array(tables[TBL_CHECK])
    tables.rindex = lexing_module.caml_lex_array(tables[TBL_RINDEX])
    tables.table = lexing_module.caml_lex_array(tables[TBL_TABLE])
    tables.len = lexing_module.caml_lex_array(tables[TBL_LEN])
    tables.lhs = lexing_module.caml_lex_array(tables[TBL_LHS])
    tables.gindex = lexing_module.caml_lex_array(tables[TBL_GINDEX])
    tables.dgoto = lexing_module.caml_lex_array(tables[TBL_DGOTO])
  end
...
function M.caml_set_parser_trace(bool)
...
function M.caml_create_parser_env(stacksize)
...
function M.caml_grow_parser_stacks(env, new_size)
...
function M.caml_parser_rule_info(env)
...
-- Returns: value
function M.caml_parser_stack_value(env, offset)
  local asp = env[ENV_ASP]
...
-- Returns: position
function M.caml_parser_symb_start(env, offset)
  local asp = env[ENV_ASP]
...
-- offset: offset from asp
-- Returns: position
function M.caml_parser_symb_end(env, offset)
  local asp = env[ENV_ASP]
