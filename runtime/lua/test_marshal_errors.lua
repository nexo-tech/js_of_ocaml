...
local ok, err = pcall(marshal.to_string, nil, {tag = 0})
...
ok, err = pcall(marshal.to_string, 42, "invalid")
...
ok, err = pcall(marshal.from_bytes, 12345, 0)
...
ok, err = pcall(marshal.from_bytes, "test", -1)
...
ok, err = pcall(marshal.from_bytes, "test", "invalid")
...
-- Test 6: Empty string
ok, err = pcall(marshal.from_bytes, "", 0)
...
ok, err = pcall(marshal.from_bytes, string.rep("\0", 10), 0)
...
local valid_string = marshal.to_string("Hello, World!", {tag = 0})
local truncated = string.sub(valid_string, 1, 25)  -- Cut off some of the string data
ok, err = pcall(marshal.from_bytes, truncated, 0)
...
ok, err = pcall(marshal.from_bytes, header_plus_one, 0)
...
ok, err = pcall(marshal.from_bytes, corrupted_magic, 0)
...
ok, err = pcall(marshal.from_bytes, invalid_code, 0)
...
local marshalled = marshal.to_string(42, {tag = 0})
...
ok, err = pcall(marshal.from_bytes, bad_shared, 0)
...
ok, err = pcall(marshal.to_string, 42, {tag = 0, [1] = 1})  -- Closures flag
...
ok, err = pcall(marshal.from_bytes, code_pointer, 0)
...
ok, err = pcall(marshal.from_bytes, block64, 0)
...
local marshalled_42 = marshal.to_string(42, {tag = 0})
ok, err = pcall(marshal.from_bytes, marshalled_42, 1000000)
...
ok, err = pcall(marshal.from_bytes, marshalled_42, #marshalled_42)
...
ok, err = pcall(marshal.from_bytes, multi_error, 0)
...
ok, err = pcall(marshal.to_string, 42, "string")
...
local m1 = marshal.to_string(10, {tag = 0})
local m2 = marshal.to_string(20, {tag = 0})
local combined = m1 .. m2

local val1 = marshal.from_bytes(combined, 0)
test("Can read first value from combined", val1 == 10)

local val2 = marshal.from_bytes(combined, #m1)
...
local simple = marshal.to_string(999, {tag = 0})
local result = marshal.from_bytes(simple, 0)
