...
test("Parse %d format", function()
  local f = format.caml_parse_format("d")
  assert_eq(f.base, 10)
...
test("Parse %i format", function()
  local f = format.caml_parse_format("i")
  assert_eq(f.base, 10)
  assert_true(f.signedconv)
  assert_eq(f.conv, "i")
end)
...
test("Parse %u format", function()
  local f = format.caml_parse_format("u")
  assert_eq(f.base, 10)
...
test("Parse %x format", function()
  local f = format.caml_parse_format("x")
  assert_eq(f.base, 16)
  assert_false(f.signedconv)
...
test("Parse %o format", function()
  local f = format.caml_parse_format("o")
  assert_eq(f.base, 8)
  assert_eq(f.conv, "o")
end)
...
test("Parse %f format", function()
  local f = format.caml_parse_format("f")
  assert_true(f.signedconv)
  assert_eq(f.conv, "f")
end)

test("Parse %e format", function()
  local f = format.caml_parse_format("e")
  assert_true(f.signedconv)
  assert_eq(f.conv, "e")
end)

test("Parse %g format", function()
  local f = format.caml_parse_format("g")
  assert_true(f.signedconv)
  assert_eq(f.conv, "g")
end)
...
test("Parse %s format", function()
  local f = format.caml_parse_format("s")
  assert_eq(f.conv, "s")
end)

test("Parse %c format", function()
  local f = format.caml_parse_format("c")
  assert_eq(f.conv, "c")
end)
...
test("Parse %0d (zero padding)", function()
  local f = format.caml_parse_format("0d")
...
test("Parse %5d (width 5)", function()
  local f = format.caml_parse_format("5d")
  assert_eq(f.width, 5)
  assert_eq(f.base, 10)
end)

test("Parse %10d (width 10)", function()
  local f = format.caml_parse_format("10d")
  assert_eq(f.width, 10)
end)

test("Parse %123d (width 123)", function()
  local f = format.caml_parse_format("123d")
  assert_eq(f.width, 123)
end)

-- Precision
test("Parse %.2f (precision 2)", function()
  local f = format.caml_parse_format(".2f")
  assert_eq(f.prec, 2)
  assert_eq(f.conv, "f")
end)

test("Parse %.5f (precision 5)", function()
  local f = format.caml_parse_format(".5f")
  assert_eq(f.prec, 5)
end)

test("Parse %.0f (precision 0)", function()
  local f = format.caml_parse_format(".0f")
  assert_eq(f.prec, 0)
end)
...
test("Parse %-10s (left justify + width)", function()
  local f = format.caml_parse_format("-10s")
  assert_eq(f.justify, "-")
  assert_eq(f.width, 10)
  assert_eq(f.conv, "s")
end)
...
test("Parse %05d (zero pad + width)", function()
  local f = format.caml_parse_format("05d")
  assert_eq(f.filler, "0")
  assert_eq(f.width, 5)
  assert_eq(f.base, 10)
end)
...
test("Parse %#8x (alternate + width)", function()
  local f = format.caml_parse_format("#8x")
  assert_true(f.alternate)
  assert_eq(f.width, 8)
  assert_eq(f.base, 16)
end)
...
test("Parse %8.2f (width + precision)", function()
  local f = format.caml_parse_format("8.2f")
  assert_eq(f.width, 8)
  assert_eq(f.prec, 2)
  assert_eq(f.conv, "f")
end)
...
test("Parse %+08d (sign + zero pad + width)", function()
  local f = format.caml_parse_format("+08d")
  assert_eq(f.signstyle, "+")
  assert_eq(f.filler, "0")
  assert_eq(f.width, 8)
  assert_eq(f.base, 10)
...
test("Parse %#010x (alternate + zero + width)", function()
  local f = format.caml_parse_format("#010x")
  assert_true(f.alternate)
  assert_eq(f.filler, "0")
  assert_eq(f.width, 10)
  assert_eq(f.base, 16)
...
test("Parse % 8.3f (space + width + precision)", function()
  local f = format.caml_parse_format(" 8.3f")
  assert_eq(f.signstyle, " ")
  assert_eq(f.width, 8)
  assert_eq(f.prec, 3)
end)
...
test("Parse %+10.5f (all modifiers)", function()
  local f = format.caml_parse_format("+10.5f")
  assert_eq(f.signstyle, "+")
  assert_eq(f.width, 10)
  assert_eq(f.prec, 5)
  assert_eq(f.conv, "f")
...
test("Format simple string", function()
  ...
  local result = format.caml_finish_formatting(f, "123")
...
test("Format with width (right justify)", function()
  local f = format.caml_parse_format("5d")
  f.sign = 1
  local result = format.caml_finish_formatting(f, "42")
...
test("Format hex with alternate form", function()
  local f = format.caml_parse_format("#x")
  f.sign = 1
  local result = format.caml_finish_formatting(f, "2a")
...
test("Format octal with alternate form", function()
  local f = format.caml_parse_format("#o")
  f.sign = 1
  local result = format.caml_finish_formatting(f, "52")
  assert_eq(ocaml_string_to_lua(result), "052")
end)

test("Format with sign and width", function()
  local f = format.caml_parse_format("+6d")
  f.sign = 1
  local result = format.caml_finish_formatting(f, "42")
  assert_eq(ocaml_string_to_lua(result), "   +42")
end)
...
test("Format with sign, zero pad, and width", function()
  local f = format.caml_parse_format("+06d")
  f.sign = 1
  local result = format.caml_finish_formatting(f, "42")
  assert_eq(ocaml_string_to_lua(result), "+00042")
end)
...
test("Format hex with alternate and width", function()
  local f = format.caml_parse_format("#8x")
  f.sign = 1
  local result = format.caml_finish_formatting(f, "1a2b")
...
test("Format uppercase conversion", function()
  local f = format.caml_parse_format("X")
  f.sign = 1
  local result = format.caml_finish_formatting(f, "abcd")
  assert_eq(ocaml_string_to_lua(result), "ABCD")
end)

test("Format long number with width", function()
  local f = format.caml_parse_format("3d")
  f.sign = 1
  local result = format.caml_finish_formatting(f, "12345")
...
test("Format string too long (>31 chars)", function()
  local success = pcall(function()
    format.caml_parse_format("01234567890123456789012345678901")
  end)
