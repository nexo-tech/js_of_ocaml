#!/usr/bin/env lua
-- Test suite for in-memory channels (Task 9.7)
--
...
local buf = {}
local bytes_read = caml_ml_input(chan_in, buf, 0, 5)
local chars = {}
...
buf = {}
bytes_read = caml_ml_input(chan_in, buf, 0, 100)
chars = {}
for i = 1, bytes_read do
  table.insert(chars, string.char(buf[i]))
end
...
local ok, err = pcall(caml_ml_input_char, chan_in)
...
ok, err = pcall(caml_ml_input_char, chan_in)
...
ok, err = pcall(caml_ml_output_char, chan_out, string.byte("A"))
...
chan_out = caml_ml_open_buffer_out()
...
caml_ml_buffer_reset(chan_out)
caml_output_value(chan_out, 10, {tag = 0})
caml_output_value(chan_out, 20, {tag = 0})
caml_output_value(chan_out, 30, {tag = 0})
contents = caml_ml_buffer_contents(chan_out)
...
local marshalled_int = caml_marshal_to_string(42, {tag = 0})
chan_in = caml_ml_open_string_in(marshalled_int)
...
local marshalled_str = caml_marshal_to_string("Hello, World!", {tag = 0})
chan_in = caml_ml_open_string_in(marshalled_str)
value = caml_input_value(chan_in)
...
local marshalled_list = caml_marshal_to_string(test_list, {tag = 0})
chan_in = caml_ml_open_string_in(marshalled_list)
value = caml_input_value(chan_in)
...
marshalled_int = caml_marshal_to_string(100, {tag = 0})
local marshalled_str2 = caml_marshal_to_string("test", {tag = 0})
local combined = marshalled_int .. marshalled_str2
chan_in = caml_ml_open_string_in(combined)
...
chan_out = caml_ml_open_buffer_out()
caml_output_value(chan_out, 999, {tag = 0})
contents = caml_ml_buffer_contents(chan_out)
caml_ml_close_channel(chan_out)

chan_in = caml_ml_open_string_in(contents)
value = caml_input_value(chan_in)
caml_ml_close_channel(chan_in)
...
chan_out = caml_ml_open_buffer_out()
caml_output_value(chan_out, "Round-trip test!", {tag = 0})
contents = caml_ml_buffer_contents(chan_out)
caml_ml_close_channel(chan_out)
...
caml_output_value(chan_out, complex, {tag = 0})
contents = caml_ml_buffer_contents(chan_out)
caml_ml_close_channel(chan_out)
...
chan_in = caml_ml_open_string_in("")
ok, err = pcall(caml_ml_input_char, chan_in)
...
chan_in = caml_ml_open_string_in(large_str)
buf = {}
bytes_read = caml_ml_input(chan_in, buf, 0, 10000)
...
chan_out = caml_ml_open_buffer_out()
for i = 1, 1000 do
  caml_ml_output_char(chan_out, string.byte("Y"))
end
contents = caml_ml_buffer_contents(chan_out)
...
str = "ABCDEFGH"
chan_in = caml_ml_open_string_in(str)
buf = {}
bytes_read = caml_ml_input(chan_in, buf, 0, 3)
chars = {}
for i = 1, bytes_read do
  table.insert(chars, string.char(buf[i]))
end
...
chan_out = caml_ml_open_buffer_out()
caml_ml_output(chan_out, "First", 0, 5)
caml_ml_buffer_reset(chan_out)
caml_ml_output(chan_out, "Second", 0, 6)
caml_ml_buffer_reset(chan_out)
caml_ml_output(chan_out, "Third", 0, 5)
contents = caml_ml_buffer_contents(chan_out)
