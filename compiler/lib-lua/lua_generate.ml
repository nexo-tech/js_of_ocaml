...
and generate_prim ctx prim args =
  ...
  | Code.Extern name, args -> (
      ...
      | "div", [ e1; e2 ] ->
          (* Integer division in Lua 5.1: math.floor(e1 / e2) *)
          L.Call (L.Ident "math.floor", [L.BinOp (L.Div, e1, e2)])
      | "mod", [ e1; e2 ] -> L.BinOp (L.Mod, e1, e2)
      (* Direct integer operations (optimized versions) *)
      | "%direct_int_mul", [ e1; e2 ] -> L.BinOp (L.Mul, e1, e2)
      | "%direct_int_div", [ e1; e2 ] ->
          (* Integer division in Lua 5.1: math.floor(e1 / e2) *)
          L.Call (L.Ident "math.floor", [L.BinOp (L.Div, e1, e2)])
      | "%direct_int_mod", [ e1; e2 ] -> L.BinOp (L.Mod, e1, e2)
